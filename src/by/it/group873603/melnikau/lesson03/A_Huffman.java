package by.it.group873603.melnikau.lesson03;
...
for(int i=0;i<s.length();i++){
    ...
    else count.replace(s.charAt(i),count.get(s.charAt(i))+1);
}
        //2. перенесем все символы в приоритетную очередь в виде листьев
PriorityQueue<Node> nodePriorityQueue= new PriorityQueue<>();
for(Map.Entry<Character,Integer> ent : count.entrySet()){
    nodePriorityQueue.add(new LeafNode((ent.getValue()),ent.getKey()));
}
...
while(nodePriorityQueue.size()>1){
    Node first_child=nodePriorityQueue.poll();
    Node second_child= nodePriorityQueue.poll();
    Node parent= new InternalNode(first_child,second_child);
    nodePriorityQueue.add(parent);
        ...
        nodePriorityQueue.peek().fillCodes("");
        for(int i=0;i<s.length();i++){
