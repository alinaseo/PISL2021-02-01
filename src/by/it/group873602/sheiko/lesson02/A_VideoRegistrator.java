...
public class A_VideoRegistrator {
    ...
    List<Double> calcStartTimes(double[] events, double workDuration){
        ...
        while (i<events.length) { //пока есть незарегистрированные события
            ...
            double endTime = events[i] + workDuration; //вычислим момент окончания работы видеокамеры
            result.add(startTime);
            while(i < events.length && events[i] <= endTime) { // и теперь пропустим все покрываемые события за время до конца работы,
                i++;// увеличивая индекс
            }
        }
